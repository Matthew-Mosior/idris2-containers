...
module Data.RRBVector1.ThreadSafe.Sized
...
(<|) :  {n : Nat}
     ...
     -> F1 s (n' ** RRBVector1 s n' a)
(x <| Empty)          t =
  let s # t := Data.RRBVector1.ThreadSafe.Sized.singleton x t
    in (1 ** s) # t
(x <| (Root sh tree)) t =
...
insertAt :  {n : Nat}
         ...
         -> F1 s (n' ** RRBVector1 s n' a)
insertAt i x v t =
  let ((tt ** left), (dt ** right)) # t := Data.RRBVector1.ThreadSafe.Sized.splitAt i v t
      (l  ** left')                 # t := ((|>) left x) t
    in (><) left' right t
...
deleteAt :  {n : Nat}
         ...
         -> F1 s (n' ** RRBVector1 s n' a)
deleteAt i v t =
  let ((tt ** left), (dt ** right)) # t := Data.RRBVector1.ThreadSafe.Sized.splitAt (plus i 1) v t
