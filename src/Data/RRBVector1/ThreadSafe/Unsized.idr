...
module Data.RRBVector1.ThreadSafe.Unsized
...
update i x v@(Root size sh tree) t =
  ...
  where
    updateTree :  Nat
               ...
               -> Tree1 s a
               -> F1 s (Tree1 s a)
    updateTree i sh tree@(Balanced (b ** arr))         t =
      case tryNatToFin (radixIndex i sh) of
        Nothing =>
        ...
        Just i' =>
          let newtree  # t := get arr i' t
              ...
              True     # t := casset arr i' newtree newtree' t
                | _ # t =>
                    assert_total $ updateTree i sh tree t
            in (Balanced {bsize=b} (b ** arr)) # t
    updateTree i sh tree@(Unbalanced (u ** arr) sizes) t =
      let (idx, subidx) := relaxedRadixIndex sizes i sh
        in case tryNatToFin idx of
             Nothing   =>
               (assert_total $ idris_crash "Data.RRBVector.update.updateTree: can't convert Nat to Fin") # t
             Just idx' =>
               let newtree  # t := get arr idx' t
                   ...
                   True     # t := casset arr idx' newtree newtree' t
                     | _ # t =>
                         assert_total $ updateTree i sh tree t
                 in (Unbalanced (u ** arr) sizes) # t
    updateTree i _  tree@(Leaf (l ** arr))             t =
      let i' = integerToNat ((natToInteger i) .&. (natToInteger blockmask))
        in case tryNatToFin i' of
             Nothing =>
             ...
             Just i'' =>
               let newtree # t := get arr i'' t
                   True    # t := casset arr i'' newtree x t
                     | _ # t =>
                         assert_total $ updateTree i sh tree t
...
adjust i f v@(Root size sh tree) t =
  ...
  where
    adjustTree :  Nat
               ...
               -> Tree1 s a
               -> F1 s (Tree1 s a)
    adjustTree i sh tree@(Balanced (b ** arr))         t =
      case tryNatToFin (radixIndex i sh) of
        Nothing =>
        ...
        Just i' =>
          let newtree  # t := get arr i' t
              ...
              True     # t := casset arr i' newtree newtree' t
                | _ # t =>
                  assert_total $ adjustTree i sh tree t
            in (Balanced {bsize=b} (b ** arr)) # t
    adjustTree i sh tree@(Unbalanced (u ** arr) sizes) t =
      let (idx, subidx) := relaxedRadixIndex sizes i sh
        in case tryNatToFin idx of
             Nothing   =>
               (assert_total $ idris_crash "Data.RRBVector.adjust: can't convert Nat to Fin") # t
...
takeTree :  Nat
         ...
         -> Tree1 s a
         -> F1 s (Tree1 s a)
takeTree i sh tree@(Balanced (b ** arr)) with (radixIndex i sh) | ((plus (radixIndex i sh) 1) <= b) proof eq
  _ | i' | True  = \t =>
    case tryNatToFin i' of
      ...
      Just i'' =>
        let arr'     # t := mtake arr (plus (radixIndex i sh) 1) @{lteOpReflectsLTE _ _ eq} t
            ...
            True     # t := casset arr' i'' newtree newtree' t
              | _ # t =>
                assert_total $ takeTree i sh tree t
...
takeTree i sh tree@(Unbalanced (u ** arr) sizes) with (relaxedRadixIndex sizes i sh) | ((plus (fst (relaxedRadixIndex sizes i sh)) 1) <= u) proof eq
  _ | (idx, subidx) | True  = \t =>
...
dropTree :  Nat
         ...
         -> Tree1 s a
         -> F1 s (Tree1 s a)
dropTree n sh tree@(Balanced (b ** arr))         t =
    ...
    in case tryNatToFin 0 of
         ...
         Just zero =>
           let arr'      # t := mdrop idx arr t
               ...
               True      # t := casset arr' zero newtree newtree' t
                 | _ # t =>
                   assert_total $ dropTree n sh tree t
               newtree'' # t := computeSizes sh arr' t
             in newtree'' # t
dropTree n sh tree@(Unbalanced (u ** arr) sizes) t =
...
insertAt :  Nat
         ...
         -> F1 s (RRBVector1 s a)
insertAt i x v t =
  let (left, right) # t := Data.RRBVector1.ThreadSafe.Unsized.splitAt i v t
      left'         # t := ((|>) left x) t
    in (><) left' right t
...
deleteAt :  Nat
         ...
         -> F1 s (RRBVector1 s a)
deleteAt i v t =
  let (left, right) # t := Data.RRBVector1.ThreadSafe.Unsized.splitAt (plus i 1) v t
