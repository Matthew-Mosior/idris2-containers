||| Finite Maps
module Data.Map.Sized

import public Data.Map.Sized.Internal
...
foldl f z (MkMap Tip)             =
  z
foldl f z (MkMap (Bin _ _ x l r)) =
  Data.Map.Sized.Internal.Tree.foldl f (f (Data.Map.Sized.Internal.Tree.foldl f z l) x) r
...
foldr f z (MkMap Tip)             =
  z
foldr f z (MkMap (Bin _ _ x l r)) =
  Data.Map.Sized.Internal.Tree.foldr f (f x (Data.Map.Sized.Internal.Tree.foldr f z r)) l
...
foldlWithKey :  (v -> k -> w -> v)
             ...
             -> Map n k w
             -> v
foldlWithKey f z (MkMap Tip)              =
  z
foldlWithKey f z (MkMap (Bin _ kx x l r)) =
...
foldrWithKey :  (k -> v -> w -> w)
             ...
             -> Map n k v
             -> w
foldrWithKey f z (MkMap Tip)              =
  z
foldrWithKey f z (MkMap (Bin _ kx x l r)) =
...
insert kx0 vx0 m =
  ...
  where
    go :  k
       ...
       -> Map n k v
       -> Map (n `plus` 1) k v
    go orig _  x (MkMap Tip)                 =
      MkMap ( Data.Map.Sized.Internal.Tree.Internal.singleton orig x
            )
    go orig kx x t@(MkMap (Bin sz ky y l r)) =
          ...
          let l'  = MkMap l
            ...
            in case l'' == l' of
                 True  =>
                   t
                 False =>
                   MkMap ( balanceL ky y (getTree l'') r
                         )
        GT =>
          let r'  = MkMap r
              r'' = go orig kx x r'
            in case r'' == r' of
                 True  =>
                   t
                 False =>
                   MkMap ( balanceR ky y l (getTree r'')
                         )
        EQ =>
          case (x == y && orig == ky) of
            True  =>
