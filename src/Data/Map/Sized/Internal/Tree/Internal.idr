||| Tree Internals
module Data.Map.Sized.Internal.Tree.Internal
...
||| An internal tree structure for a map.
public export
data Tree : (k : Type) -> (v : Type) -> Type where
  Bin :  Size
      -> k
      -> v
      -> Tree k v
...
public export
data MinView k a = MinView' k a (Tree k a)

%runElab derive "MinView" [Eq,Ord,Show]

public export
data MaxView k a = MaxView' k a (Tree k a)
...
||| The number of elements in the tree. O(1)
export
size :  Tree k v
     -> Nat
size Tip             =
...
||| Glues two trees together (assumes that both trees are already balanced with respect to each other).
export
glue :  Tree k v
     -> Tree k v
     -> Tree k v
